...
def train(opt):
    ...
    ##########################
    if opt.noamopt:
        assert opt.caption_model in ['transformer', 'bert', 'm2transformer', 'tfvae'], 'noamopt can only work with transformer'
    ...
    #  Build Mix precision
    ##########################
    mp_level = getattr(opt, 'mp_level', 'O1')
    if isinstance(optimizer, torch.optim.Optimizer):
        model, optimizer = amp.initialize(model, optimizer, opt_level=mp_level)
    else:
        model, optimizer.optimizer = amp.initialize(model, optimizer.optimizer, opt_level=mp_level)
