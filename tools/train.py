...
def train(opt):
    ...
    ##########################
    if opt.noamopt:
        assert opt.caption_model in ['transformer', 'bert', 'm2transformer', 'tfvae', 'vat'], 'noamopt can only work with transformer'
    ...
    if opt.start_from is not None and os.path.isfile(os.path.join(opt.start_from,"optimizer.pth")):
        pth = torch.load(os.path.join(opt.start_from, 'optimizer.pth'))
    ...
    try:
        while True:
            ...
            optimizer.zero_grad()
            model_out = dp_lw_model(fc_feats, att_feats, labels, masks, att_masks, data['gts'], torch.arange(0, len(data['gts'])), sc_flag, struc_flag, opt.sample_logprobs)

            ae_loss = model_out['loss'].mean()
            # for transform vae
            if 'analytic_kl' in model_out:
                loss = ae_loss + opt.kl_beta * model_out['analytic_kl'].mean()
            else:
                loss = ae_loss
            ...
            if (iteration % opt.losses_log_every == 0):
                ...
                # for transform vae
                if 'analytic_kl' in model_out:
                    tb_summary_writer.add_scalar('analytic_kl', model_out['analytic_kl'].mean().item(), iteration)
                if 'pa_loss' in model_out:
                    tb_summary_writer.add_scalar('pa_loss', model_out['pa_loss'].mean().item(), iteration)
...
# import sys
# os.chdir('..')
# sys.argv = [sys.argv[0], '--cfg', 'configs/vat/vat_nscl.yml', '--id', 'vat01_nscl']
opt = opts.parse_opt()
train(opt)
