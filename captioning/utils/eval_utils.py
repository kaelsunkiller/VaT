...
def language_eval(dataset, preds, preds_n, eval_kwargs, split):
    ...
    if len(preds_n) > 0:
        ...
        if 'coco' in dataset:
            dataset_file = '../Self-critical.pytorch/data/dataset_coco.json'
        elif 'flickr30k' in dataset or 'f30k' in dataset:
        ...
        for pred in preds_n:
            ...
            preds_n_dict[img_id] = set(list(list(preds_n_dict[img_id]) + [pred['caption']])) if img_id in preds_n_dict else [pred['caption']]
...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            # parallel decoder
            # pa_seq, _ = model(fc_feats, att_feats, seq, att_masks, opt=tmp_eval_kwargs, mode='pa_sample')
            # pa_seq = pa_seq.data
        ...
        # parallel decoder
        # pa_sents = utils.decode_sequence(model.vocab, pa_seq)
        # pa_predictions = []
        # for k, sent in enumerate(pa_sents):
        #     pa_entry = {'image_id': data['infos'][k]['id'], 'caption': sent, 'perplexity': perplexity[k].item(), 'entropy': entropy[k].item()}
        #     if eval_kwargs.get('dump_path', 0) == 1:
        #         pa_entry['file_name'] = data['infos'][k]['file_path']
        #     pa_predictions.append(pa_entry)
        #     if verbose:
        #         print('pa image %s: %s' %(pa_entry['image_id'], pa_entry['caption']))
    ...
    if lang_eval == 1:
        ...
        # parallel decoder
        # pa_lang_stats = language_eval(dataset, pa_predictions, [], eval_kwargs, split)
        # for k, v in pa_lang_stats.items():
        #     lang_stats['pa_'+k] = v
...
def eval_split_n(model, n_predictions, input_data, eval_kwargs={}):
    ...
    if sample_n_method == 'bs':
        ...
        for k in range(fc_feats.shape[0]):
            ...
            _sents = utils.decode_sequence(model.vocab, nn.utils.rnn.pad_sequence(
