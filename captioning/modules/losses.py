...
class StructureLosses(nn.Module):
    ...
    def forward(self, input, seq, data_gts):
        ...
        # input: BxTxvoc_num, seq:BxT, data_gts:bxseq_per_imgxsen_len, 10x5x16, B=bxseq_per_img
        batch_size = input.size(0)# batch_size = sample_size * seq_per_img
        seq_per_img = batch_size // len(data_gts)
        voc_num = input.size(-1)
        ...
        scores = torch.from_numpy(scores).type_as(input).view(-1, seq_per_img)  # bxseq_per_img, 10x5
        ...
        if self.loss_type == 'seqnll':
            ...
            """
            if self.opt.sc_baseline == 'avg':
                baseline = (scores.sum(1, keepdim=True) - scores) / (scores.shape[1] - 1)
            elif self.opt.sc_baseline == 'mid':
                baseline = (scores.max(1, keepdims=True)[0] + scores.min(1, keepdims=True)[0]) / 2
            elif self.opt.sc_baseline == 'mid2':
                scores_ = scores.new_full((*scores.shape, scores.shape[1]-1), 0.)
                for i in range(scores.shape[1]):
                    scores_[:, i, :i] = scores[:, :i]
                    scores_[:, i, i:] = scores[:, i+1:]
                baseline = (scores_.max(-1)[0] + scores_.min(-1)[0]) / 2
            elif self.opt.sc_baseline == 'mix':
                ...
                baseline = torch.where(scores_mid.abs() > scores_avg.abs(), mid, avg)
            else:
                raise Exception(self.opt.sc_baseline, 'are not supported')
            ...
            if self.opt.pos_only:
                # seq_scores = get_scores(seq.view_as(data_gts), seq, self.opt)
                # seq_scores = torch.from_numpy(seq_scores).type_as(input).view(-1, seq_per_img)  # bxseq_per_img, 10x5
                scores = torch.where(scores > 0, scores, seq_scores)
            # for ghm loss
            if self.use_ghm:
                output = self.ghmloss(-input, seq, mask, scores.view(-1, 1).expand(input.size())) * scores.view(-1, 1)
                # p_mask = self.ghmloss.mask_select(scores, seq, mask, voc_num)
                # output = - input * p_mask * scores.view(-1, 1)
            else:
