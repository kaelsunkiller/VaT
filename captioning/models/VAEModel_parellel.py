...
class VAE(nn.Module):
    ...
    def __init__(self, att_feats_encoder, gt_encoder, condition_encoder, decoder, ge_mlp, ce_mlp, tgt_embed, generator):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        ...
        random_embed_mask = tgt.new_ones(tgt.shape[:2], dtype=torch.long).unsqueeze(-2)

        tgt_embed = self.tgt_embed(tgt)
        random_embed = torch.randn_like(tgt_embed)
        gt_distribution_params = self.ge_mlp(self.gt_encoder(tgt_embed, memory, src_mask, tgt_full_mask))
        c_distribution_params = self.ce_mlp(self.condition_encoder(random_embed, memory, src_mask, random_embed_mask))
        ...
        decoder_memo_masks = torch.cat([src_mask, random_embed_mask.type_as(src_mask)], -1)
        out = self.decoder(self.tgt_embed(tgt), decoder_memo, decoder_memo_masks, tgt_mask)
        return out, analytic_kl

    def get_gt_z_params(self, src, tgt, src_mask, tgt_full_mask):
        memory = self.encode(src, src_mask)
    ...
    def get_c_z_params(self, src, src_mask, random_embed_mask):
        memory = self.encode(src, src_mask)
        random_embed = torch.zeros((random_embed_mask.shape[0], random_embed_mask.shape[-1], memory.shape[-1])).to(random_embed_mask.device)
        c_distribution_params = self.ce_mlp(self.condition_encoder(random_embed, memory, src_mask, random_embed_mask))
        return c_distribution_params

    def encode(self, src, src_mask):
        return self.src_encoder(src, src_mask)

    def decode(self, memory, src_mask, tgt, tgt_mask, max_seq_length):
        random_embed_mask = tgt.new_ones([tgt.shape[0], 1, max_seq_length], dtype=torch.long)
        random_embed = torch.zeros((random_embed_mask.shape[0], random_embed_mask.shape[-1], memory.shape[-1])).to(random_embed_mask.device)
        c_distribution_params = self.ce_mlp(self.condition_encoder(random_embed, memory, src_mask, random_embed_mask))
        c_mean, c_logvar = c_distribution_params[..., 0], c_distribution_params[..., 1]
    ...
    def l_decode(self, z_params, memory, src_mask, tgt, tgt_mask, mode='gt'):
        mean, logvar = z_params[..., 0], z_params[..., 1]
        epsilon = torch.randn(mean.shape).to(mean.device)
        sample_z = epsilon * torch.exp(.5 * logvar) + mean
        if self.gmm_num > 1:
            if 'gt' == mode:
                gmmn = torch.softmax(self.gmmn_gt, dim=0)
            elif 'c' == mode:
                gmmn = torch.softmax(self.gmmn_c, dim=0)
            else:
                raise Exception("decoding mode error. {} not in ['gt', 'c'].".format(mode))
            sample_z = sample_z.index_select(0, torch.multinomial(gmmn.squeeze(), 1)).squeeze()
        if sample_z.ndim == 2:
            sample_z = sample_z.unsqueeze(1)

        random_embed_mask = tgt.new_ones(sample_z.shape[:2], dtype=torch.long).unsqueeze(-2)
...
class ParallelDecoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        for i in range(x.size(1)):
            out = self.decoder(ys, memory, src_mask, subsequent_mask(ys.size(1)).to(memory.device))
            ys = torch.cat([ys, out[:, -1:]], 1)
        return ys[:, 1:]


class TFVAEModel_parallel(TransformerModel):
    ...
    def __init__(self, opt):
        super(TFVAEModel_parallel, self).__init__(opt)
    ...
    def core_forward(self, it, gt_z_params, memory, state, mask):
        ...
        out = self.model.l_decode(gt_z_params, memory, mask, ys, subsequent_mask(ys.size(1)).to(memory.device))
    ...
    #     seq_full_mask = fc_feats.new_ones((batch_size, 1, self.seq_length), dtype=torch.long)
    #     c_z_params = self.model.get_c_z_params(p_att_feats, p_att_masks, seq_full_mask)
    #
    #     if sample_n > 1:
    #         p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, c_z_params = utils.repeat_tensors(sample_n,
    #                                                                                   [p_fc_feats, p_att_feats,
    #                                                                                    pp_att_feats, p_att_masks,
    #                                                                                    c_z_params.permute(1, 0, 2, 3, 4)]
    #                                                                                   )
    #         c_z_params = c_z_params.permute(1, 0, 2, 3, 4)
    #
    #     trigrams = []  # will be a list of batch_size dictionaries
    #
    ...
    #     self.logprobs_all = self.get_logprobs_state_forward(c_z_params, pp_att_feats, p_att_masks,
    #                                                    output_logsoftmax=output_logsoftmax, mode='c')
    #
    #     if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
    #         self.logprobs_all = utils.repeat_tensors(beam_size, self.logprobs_all)
    #         return self._sample_beam(fc_feats, att_feats, att_masks, opt)
    #     if group_size > 1:
    #         return self._diverse_sample(fc_feats, att_feats, att_masks, opt)
    #
